...
function getDatabase() {
    ...
    // 1. Try local uncompressed DB first
    const localDbPath = path.join(process.cwd(), 'backend', 'thanawya.db');
    if (fs.existsSync(localDbPath)) {
        console.log('Connected to local Thanawya SQLite DB at:', localDbPath);
        dbInstance = new sqlite3.Database(localDbPath, sqlite3.OPEN_READONLY);
        return dbInstance;
    }

    // 2. Fallback to /tmp decompressed DB for Vercel / serverless environment
    const tmpDbPath = path.join('/tmp', 'thanawya.db');
    if (!fs.existsSync(tmpDbPath)) {
        console.log('Decompressing thanawya.db.gz to /tmp...');
        const gzPath = path.join(process.cwd(), 'backend', 'thanawya.db.gz');
        if (fs.existsSync(gzPath)) {
            const compressed = fs.readFileSync(gzPath);
            const decompressed = zlib.gunzipSync(compressed);
            fs.writeFileSync(tmpDbPath, decompressed);
            console.log('Successfully decompressed DB to /tmp, size:', decompressed.length);
        } else {
            console.error('Database compressed file not found at:', gzPath);
        }
    } else {
        console.log('Found cached decompressed DB at:', tmpDbPath);
    }
    ...
    dbInstance = new sqlite3.Database(tmpDbPath, sqlite3.OPEN_READONLY, (err) => {
        if (err) {
            ...
            console.log('Connected to Thanawya SQLite DB at:', tmpDbPath);
        }
    });
